...
const siteOrigin = nconf.get('url');
...
const getGcs = memoizeOne(
    params => new Storage(params),
    isEqual,
);
...
class Controllers {
    ...
    static ipwSettings(req, res) {
        ...
        const settings = pick(pluginSettings.get(), ['gcs', 'rmq']);

        Object.assign(
            settings,
            {
                ipw: pick(pluginSettings.get('ipw'), ['version', 'transport']),
            },
        );
    ...
    static download(req, res) {
        ...
        function streamResponse(data, callback) {
            ...
            bucket.file(data.filename).createReadStream()
                ...
                .on('error', (err) => {
                    callback(new Error(`Google API Error: ${err.message}`));
                })
                .pipe(res);
        }
    ...
    static postParser(post, next) {
        ...
        async.eachSeries($images, (imageEl, callback) => {
            ...
            async.eachSeries($photoset, (imageEl, cb) => {
                ...
                /* remove proto://host */
                const imageSrc = $image.attr('src').replace(siteOrigin, '');
                $image.attr('src', imageSrc);

                const urlParser = imageFilename.urlParser(imageSrc);
                ...
                const $link = $(`<a href="${imageSrc}" target="_blank"></a>`);
