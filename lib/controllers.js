...
Controllers.upload = function( req, res ) {
    ...
    var queue = async.queue( function( readStream, callback ) {
        ...
        {
            isAllowed =
                ~uploadsSetting.allowedImageTypes.indexOf( ext )
                && (readStream.byteCount <= uploadsSetting.allowedImageMaxFileSize);
        }
        else
        {
            isAllowed =
                ~uploadsSetting.allowedFileTypes.indexOf( ext )
    ...
    queue.drain = function() {
        async.map( files, function( fileinfo, callback ) {
            ...
            if( fileinfo.isImage )
            {
                ...
                urlFilename = imageFilename.autoOrientation( thumbnailFilename, fileinfo.exifOrientation, true );
            }
            else
            {
...
Controllers.download = function( req, res ) {
    ...
    async.waterfall( [
        async.constant( {
            ...
            isImage: /^image/.test( mime.extension( urlParser.ext.replace( '.', '' ) ) )
        } ),
        validateHmac,
        isRemoteFileExists,
        isResizeTaskExists,
