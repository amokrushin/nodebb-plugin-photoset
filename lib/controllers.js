...
Controllers.upload = function( req, res ) {
    ...
    queue.drain = function() {
        async.map( files, function( file, callback ) {
            var filename = file.filename + file.ext,
            // todo: imagePreviewMaxDimension
                thumbnailFilename = imageFilename.thumbnail( filename, photosetSettings.imagePreviewMaxDimension || 800 );

            thumbnailFilename = imageFilename.autoOrientation( thumbnailFilename, file.exifOrientation, true );
            filenameHmac( photosetSettings.hmac.key, thumbnailFilename + file.ext, function( err, hmac ) {
                callback( null, {
